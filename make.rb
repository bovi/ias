...
# Building folder
BUILD_DIR = "/home/daniel/Documents/ias/build"
...
INCLUDES = "-I#{SAM_DIR}/system/libsam -I#{SAM_DIR}/system/CMSIS/CMSIS/Include/ -I#{SAM_DIR}/system/CMSIS/Device/ATMEL/ -I#{SAM_DIR}/cores/arduino -I#{TARGET_DIR} -Imruby/include"
...
execute "cp -Rf ias.cpp #{BUILD_DIR}/ias.cpp"
execute "cp -Rf libmruby.a #{BUILD_DIR}/libmruby.a"

USER_FILES = %w(ias.cpp)
...
execute "#{BIN_DIR}/arm-none-eabi-g++ -Os -Wl,--gc-sections -mcpu=cortex-m3 -T#{TARGET_DIR}/linker_scripts/gcc/flash.ld -Wl,-Map,#{BUILD_DIR}/ias.cpp.map -o #{BUILD_DIR}/ias.cpp.elf -L#{BUILD_DIR} -lm -lgcc -mthumb -Wl,--cref -Wl,--check-sections -Wl,--gc-sections -Wl,--entry=Reset_Handler -Wl,--unresolved-symbols=report-all -Wl,--warn-common -Wl,--warn-section-align -Wl,--warn-unresolved-symbols -Wl,--start-group #{BUILD_DIR}/libmruby.a #{BUILD_DIR}/syscalls_sam3.c.o #{BUILD_DIR}/ias.cpp.o #{TARGET_DIR}/libsam_sam3x8e_gcc_rel.a #{BUILD_DIR}/core.a -Wl,--end-group"

# Build Binary for target
puts "PACK"
execute "#{BIN_DIR}/arm-none-eabi-objcopy -O binary #{BUILD_DIR}/ias.cpp.elf #{BUILD_DIR}/ias.cpp.bin"
...
if FLASH
  ...
  execute "#{ARDUINO_DIR}/hardware/tools/bossac --port=#{USB_PORT} -U false -e -w -v -b #{BUILD_DIR}/ias.cpp.bin -R"
end
